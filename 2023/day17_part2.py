...
def process_all(lines):
    ...
    def neighbours(args):
        ...
        if remaining_count <= 10 - 4:
            if current_dir in [LEFT, RIGHT]:
                cost = [get_score(position[0], position[1] - d) for d in range(1, 4 + 1)]
                if None not in cost:
                    candidates.append(((position[0], position[1] - 1), UP, 9))

                cost = [get_score(position[0], position[1] + d) for d in range(1, 4 + 1)]
                if None not in cost:
                    candidates.append(((position[0], position[1] + 1), DOWN, 9))
            elif current_dir in [UP, DOWN]:
                cost = [get_score(position[0] - d, position[1]) for d in range(1, 4 + 1)]
                if None not in cost:
                    candidates.append(((position[0] - 1, position[1]), LEFT, 9))

                cost = [get_score(position[0] + d, position[1]) for d in range(1, 4 + 1)]
                if None not in cost:
                    candidates.append(((position[0] + 1, position[1]), RIGHT, 9))
