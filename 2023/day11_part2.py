...
def process_all(lines):
    ...
    def compute_distance(g1, g2):
        ...
        d += len(list(x for x in range(min(g1[0], g2[0]) + 1, max(g1[0], g2[0])) if x in xs_without_galaxy)) * (1000000 - 1)
        d += len(list(y for y in range(min(g1[1], g2[1]) + 1, max(g1[1], g2[1])) if y in ys_without_galaxy)) * (1000000 - 1)
        return d
