...
def process_all(lines):
    ...
    for x in range(len(lines[0])):
        result = max(result, compute(((x, 0), DOWN)))
        result = max(result, compute(((x, len(lines) - 1), UP)))

    for y in range(len(lines)):
        result = max(result, compute(((0, y), RIGHT)))
        result = max(result, compute(((len(lines[0]) - 1, y), LEFT)))
    return result
