...
def process_all(lines):
    def compute_type(hand):
        most_commons = sorted(map(lambda x: x[1], Counter(hand).most_common(2)), reverse=True)
        print('most_commons', hand, most_commons)
        if most_commons[0] == 5:
            return 5
    ...
    def compare_hands(hand1, hand2):
        type1 = compute_type(hand1)
        type2 = compute_type(hand2)
