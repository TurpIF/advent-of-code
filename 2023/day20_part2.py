...
def process_all(lines):
    ...
    def click_once():
        nb_low = 0
        nb_high = 0
        for num_button in islice(itertools.count(start=1), 5000): # With my input, we don't need to go further
            # print()
            ...
            while open_set:
                ...
                if button in ['vt', 'sk', 'xc', 'kk'] and signal == LOW:
                    # there is:
                    # &vt -> tj; &sk -> tj; &xc -> tj; &kk -> tj and &tj -> rx
                    # So we need to find were all get LOW which will trigger rw=LOW
                    # Assuming each are isolated and perfectly cyclic without offset from start
                    # we can do a LCM with first iteration where we find each
                    # vt: 3943
                    # sk: 3917
                    # xc: 4057
                    # kk: 3931
                    # https://www.wolframalpha.com/input?i=lcm%283943%2C+3917%2C+4057%2C+3931%29
